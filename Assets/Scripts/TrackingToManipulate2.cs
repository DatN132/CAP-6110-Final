...
public class TrackingToManipulate2 : MonoBehaviour
{
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (indication != null)
        {
            ...
            if (originalParent.name == "Exodia (Person Male)")
            {
              GameObject tv = GameObject.Find("Panel");
              VideoPlayer vp = tv.GetComponent<VideoPlayer>();
              vp.Pause();
            }

            if (originalParent.name == "MR (TV)")
            {
              GameObject tv = GameObject.Find("Panel");
              tv.SetActive(false);
